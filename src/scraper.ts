...
class Scraper {
  ...
  private async getPagestoScrap(): Promise<void> {
    try {
      ...
      console.log(`numbers of pages to scrap: ${lastPageNumber} from topic: ${this.companiesTopic}`);
      ...
      console.error(
        `getPagesToScrap failed, error: ${error}`
      );
    }
  }
  ...
  private async getCompaniesUrl(): Promise<void>{
    try {
      ...
      console.log(`get companies urls from topic: ${this.companiesTopic} finished, there are ${this.companiesUrlArray.length} companies urls to scrap`)
...
try {
  console.log('Scrape data started')
    ...
    for(let companieUrl of this.companiesUrlArray){
      ...
      const email = $('div.col-lg-4.my-2 > div > div > p').first().next().text()
    ...
    console.log(`scraping data successful, a total of ${this.dataCompaniesArray.length} companies has been scraped`)
